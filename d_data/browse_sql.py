...
try:
    ...
    for root, dirs, files in os.walk(parent_dir):
        ...
        module_path1 = os.path.join(root, 'login_sql_core.py')
        module_path2 = os.path.join(root, 'a_2_sql_login.py') # placeholder for 2nd sql db credentials
        if os.path.isfile(module_path1):
            # Add the parent folder of the module to the Python path
            sys.path.append(os.path.dirname(module_path1))
            break
        if os.path.isfile(module_path2):
            # Add the parent folder of the module to the Python path
            sys.path.append(os.path.dirname(module_path2))
            break

    # Import the module from the subfolder
    try:
        from login_sql_core import get_connection
        ...
        for path in sys.path:
            print(path)
    try:
        from a_2_sql_login import get_connection
    except ModuleNotFoundError:
        print("The module a_2_sql_login could not be found.")
        print("Python search paths:")
        for path in sys.path:
            print(path)
    ...
    def main():
        ...
        if conn:
            ...
            sql_script = 'SELECT TOP 10 * FROM dbo.DimCustomer'  # Customize your SQL query here to fetch all columns
            try:
